...
const movementBank: Record<UserLevel, TargetItem[]> = {
  ...
  4: [
    { target: '굴러가다', meaning: 'катиться туда', hintKr: '둥근 물체가 굴러 멀어짐', exampleKr: '공이 담장 쪽으로 굴러갔어요.', exampleRu: 'Мяч покатился к забору.' },
    ...
    { target: '날아가다', meaning: 'улетать туда', hintKr: '공중으로 날아 멀어짐', exampleKr: '모자가 모래밭 쪽으로 날아갈 거예요.', exampleRu: 'Шапка улетит к песку.' },
    { target: '날아오다', meaning: 'прилетать сюда', hintKr: '공중에서 화자 쪽으로 옴', exampleKr: '종이비행기가 제 책상 위로 날아왔어요.', exampleRu: 'Бумажный самолётик прилетел на мой стол.' },
    { target: '뛰어가다', meaning: 'бежать туда', hintKr: '뛰어서 멀어짐', exampleKr: '민수가 정류장 쪽으로 뛰어가요.', exampleRu: 'Минсу бежит к остановке.' },
    { target: '뛰어오다', meaning: 'бежать сюда', hintKr: '뛰어서 화자 쪽으로 옴', exampleKr: '친구가 제 쪽으로 뛰어올 거예요.', exampleRu: 'Друг прибежит ко мне.' },
    { target: '걸어가다', meaning: 'идти пешком туда', hintKr: '걸어서 멀어짐', exampleKr: '우리는 찻집까지 걸어갔어요.', exampleRu: 'Мы дошли пешком до чайной.' },
    { target: '걸어오다', meaning: 'идти пешком сюда', hintKr: '걸어서 화자 쪽으로 옴', exampleKr: '친구가 제자리로 걸어와요.', exampleRu: 'Друг идёт пешком ко мне.' },
    { target: '쫓아가다', meaning: 'догонять / бежать следом туда', hintKr: '앞에 있는 대상을 따라 멀어짐', exampleKr: '강아지가 고양이 뒤를 쫓아갈 거예요.', exampleRu: 'Щенок побежит следом за кошкой.' },
    { target: '쫓아오다', meaning: 'догонять / бежать следом сюда', hintKr: '대상을 따라 화자 쪽으로 옴', exampleKr: '친구가 준이 뒤를 쫓아왔어요.', exampleRu: 'Друг побежал следом за Джуни сюда.' },
  ],
  5: [
...
function movementLevel4Context(
  ...
  const mainTemplates = [
    {
      blank: '동네 아이들이 빈 마당에서 공을 찼어요. 준호가 찬 공이 낮은 돌에 맞았어요. 공은 풀밭을 지나 담장 쪽으로 ___. 아이들은 공을 잃어버릴까 봐 얼른 따라갔어요. 결국 민아가 담장 앞에서 공을 주웠어요.',
      full: '동네 아이들이 빈 마당에서 공을 찼어요. 준호가 찬 공이 낮은 돌에 맞았어요. 공은 풀밭을 지나 담장 쪽으로 굴러갔어요. 아이들은 공을 잃어버릴까 봐 얼른 따라갔어요. 결국 민아가 담장 앞에서 공을 주웠어요.',
    },
    {
      blank: '비가 그친 뒤 골목 바닥에 물이 남았어요. 저는 작은 가게 앞에서 우산을 접고 있었어요. 누군가 떨어뜨린 동전이 물길을 따라 제 발밑으로 ___. 동생은 그 동전이 자기 것인지 물었어요. 저는 주인을 찾아 주려고 주변을 살폈어요.',
      full: '비가 그친 뒤 골목 바닥에 물이 남았어요. 저는 작은 가게 앞에서 우산을 접고 있었어요. 누군가 떨어뜨린 동전이 물길을 따라 제 발밑으로 굴러와요. 동생은 그 동전이 자기 것인지 물었어요. 저는 주인을 찾아 주려고 주변을 살폈어요.',
    },
    {
      blank: '바닷가에 바람이 점점 세지고 있어요. 지훈이는 모자를 손에 들고 사진을 찍으려고 해요. 손을 놓으면 모자가 먼 모래밭 쪽으로 ___. 친구는 그래서 모자를 가방에 넣으라고 말했어요. 지훈이는 웃으면서 고개를 끄덕였어요.',
      full: '바닷가에 바람이 점점 세지고 있어요. 지훈이는 모자를 손에 들고 사진을 찍으려고 해요. 손을 놓으면 모자가 먼 모래밭 쪽으로 날아갈 거예요. 친구는 그래서 모자를 가방에 넣으라고 말했어요. 지훈이는 웃으면서 고개를 끄덕였어요.',
    },
    {
      blank: '창가에서 저는 조용히 그림을 그리고 있었어요. 밖에서는 아이들이 종이비행기를 접어 던졌어요. 하얀 종이비행기 하나가 열린 창문으로 ___. 저는 깜짝 놀랐지만 모양이 예뻐서 웃었어요. 아이들은 미안하다고 손을 흔들었어요.',
      full: '창가에서 저는 조용히 그림을 그리고 있었어요. 밖에서는 아이들이 종이비행기를 접어 던졌어요. 하얀 종이비행기 하나가 열린 창문으로 날아왔어요. 저는 깜짝 놀랐지만 모양이 예뻐서 웃었어요. 아이들은 미안하다고 손을 흔들었어요.',
    },
    {
      blank: '아침에 민수는 시계를 보고 깜짝 놀랐어요. 버스가 곧 오는데 아직 집 앞 길에 있어요. 민수는 가방을 고쳐 메고 정류장 쪽으로 ___. 엄마는 넘어지지 말라고 뒤에서 말해요. 다행히 민수는 버스가 떠나기 전에 닿을 수 있어요.',
      full: '아침에 민수는 시계를 보고 깜짝 놀랐어요. 버스가 곧 오는데 아직 집 앞 길에 있어요. 민수는 가방을 고쳐 메고 정류장 쪽으로 뛰어가요. 엄마는 넘어지지 말라고 뒤에서 말해요. 다행히 민수는 버스가 떠나기 전에 닿을 수 있어요.',
    },
    {
      blank: '저는 학교 문 앞에서 친구를 기다리고 있어요. 멀리 운동장 옆에서 제 이름을 부르는 소리가 들려요. 친구는 저를 보면 손을 흔들며 제 쪽으로 ___. 저는 친구에게 빌려준 공책을 받을 거예요. 우리는 함께 교실로 들어갈 거예요.',
      full: '저는 학교 문 앞에서 친구를 기다리고 있어요. 멀리 운동장 옆에서 제 이름을 부르는 소리가 들려요. 친구는 저를 보면 손을 흔들며 제 쪽으로 뛰어올 거예요. 저는 친구에게 빌려준 공책을 받을 거예요. 우리는 함께 교실로 들어갈 거예요.',
    },
    {
      blank: '주말 아침에 가족이 가까운 산책길로 나왔어요. 차를 타기에는 길이 짧고 날씨도 맑았어요. 우리는 강가 길을 따라 작은 찻집까지 ___. 가는 동안 강아지도 천천히 냄새를 맡았어요. 도착한 뒤 모두 따뜻한 차를 마셨어요.',
      full: '주말 아침에 가족이 가까운 산책길로 나왔어요. 차를 타기에는 길이 짧고 날씨도 맑았어요. 우리는 강가 길을 따라 작은 찻집까지 걸어갔어요. 가는 동안 강아지도 천천히 냄새를 맡았어요. 도착한 뒤 모두 따뜻한 차를 마셨어요.',
    },
    {
      blank: '수업이 끝난 뒤 저는 교문 앞 의자에 앉아 있어요. 친구는 책을 빌리고 뒤늦게 나오고 있어요. 친구가 책가방을 메고 제자리로 천천히 ___. 저는 친구에게 오늘 빌린 책이 재미있냐고 물어요. 우리는 잠깐 이야기한 뒤 집으로 가요.',
      full: '수업이 끝난 뒤 저는 교문 앞 의자에 앉아 있어요. 친구는 책을 빌리고 뒤늦게 나오고 있어요. 친구가 책가방을 메고 제자리로 천천히 걸어와요. 저는 친구에게 오늘 빌린 책이 재미있냐고 물어요. 우리는 잠깐 이야기한 뒤 집으로 가요.',
    },
    {
      blank: '공원 길에 작은 강아지 한 마리가 뛰고 있어요. 앞쪽에서는 고양이가 풀숲으로 빠르게 달려가요. 줄을 놓으면 강아지가 고양이 뒤를 ___. 주인은 그래서 줄을 두 손으로 꼭 잡고 있어요. 고양이는 놀라지 않고 천천히 나무 뒤로 숨어요.',
      full: '공원 길에 작은 강아지 한 마리가 뛰고 있어요. 앞쪽에서는 고양이가 풀숲으로 빠르게 달려가요. 줄을 놓으면 강아지가 고양이 뒤를 쫓아갈 거예요. 주인은 그래서 줄을 두 손으로 꼭 잡고 있어요. 고양이는 놀라지 않고 천천히 나무 뒤로 숨어요.',
    },
    {
      blank: '저녁에 골목에서 아이들이 숨바꼭질을 했어요. 준이는 친구가 숨은 곳을 눈치챘어요. 친구는 웃으며 준이 뒤를 ___. 준이는 놀라서 집 앞까지 달렸어요. 두 아이는 숨을 고른 뒤 다시 놀이를 이어 갔어요.',
      full: '저녁에 골목에서 아이들이 숨바꼭질을 했어요. 준이는 친구가 숨은 곳을 눈치챘어요. 친구는 웃으며 준이 뒤를 쫓아왔어요. 준이는 놀라서 집 앞까지 달렸어요. 두 아이는 숨을 고른 뒤 다시 놀이를 이어 갔어요.',
    },
  ];

  const remedialTemplates = [
    {
      blank: '미술 시간에 아이들이 둥근 찰흙 공을 만들었어요. 한 아이가 책상 끝에 공을 올려두었어요. 찰흙 공은 손이 닿자 바닥으로 떨어져 문 쪽으로 ___. 아이들은 바닥이 더러워질까 봐 바로 주웠어요. 선생님은 물건을 끝에 두지 말라고 했어요.',
      full: '미술 시간에 아이들이 둥근 찰흙 공을 만들었어요. 한 아이가 책상 끝에 공을 올려두었어요. 찰흙 공은 손이 닿자 바닥으로 떨어져 문 쪽으로 굴러갔어요. 아이들은 바닥이 더러워질까 봐 바로 주웠어요. 선생님은 물건을 끝에 두지 말라고 했어요.',
    },
    {
      blank: '저는 놀이터 모래밭 옆에 앉아 있어요. 어린아이가 빨간 공을 살짝 밀어요. 공은 작은 언덕을 내려와 제 앞까지 ___. 아이가 공을 찾으러 오자 저는 공을 건네줘요. 아이는 고맙다고 말하고 다시 놀아요.',
      full: '저는 놀이터 모래밭 옆에 앉아 있어요. 어린아이가 빨간 공을 살짝 밀어요. 공은 작은 언덕을 내려와 제 앞까지 굴러와요. 아이가 공을 찾으러 오자 저는 공을 건네줘요. 아이는 고맙다고 말하고 다시 놀아요.',
    },
    {
      blank: '소풍 날에 아이들이 잔디밭에서 연을 날리고 있어요. 갑자기 바람이 더 세게 불어요. 줄을 놓치면 연 하나가 먼 나무 쪽으로 ___. 아이들은 연이 나뭇가지에 걸릴까 봐 걱정해요. 선생님은 줄을 꼭 잡으라고 말해요.',
      full: '소풍 날에 아이들이 잔디밭에서 연을 날리고 있어요. 갑자기 바람이 더 세게 불어요. 줄을 놓치면 연 하나가 먼 나무 쪽으로 날아갈 거예요. 아이들은 연이 나뭇가지에 걸릴까 봐 걱정해요. 선생님은 줄을 꼭 잡으라고 말해요.',
    },
  ];
...
function makeBlankSentence(target: string, exampleKr: string): string {
  ...
  if (isVerb) {
    ...
    // 가 → 갔 / 갈  (가다 compounds: 굴러가다→굴러갔어요, 굴러갈 거예요)
    if (stem.endsWith('가')) {
      const pre = stem.slice(0, -1);
      candidates.push(pre + '갔', pre + '갈');
    }

    // 오 → 와 / 왔 / 올  (오다 compounds: 굴러오다→굴러와요, 굴러왔어요, 굴러올 거예요)
    if (stem.endsWith('오')) {
      const pre = stem.slice(0, -1);
      candidates.push(pre + '와', pre + '왔', pre + '올');
    }

    // 하 → 합 / 해 / 했 / 할  (하다 compounds: 이동하다→이동합니다, 이동해요, 이동했어요, 이동할 거예요)
    if (stem.endsWith('하')) {
      const pre = stem.slice(0, -1);
...
function generateMovementLevel4Explanation(
  ...
  const explanations: Record<string, string> = {
    'main-1':
      `В этом вопросе мяч уже покатился к забору.
` +
      `Он круглый и движется по земле, поэтому нужен глагол «굴러갔어요».
` +
      `Часть «갔어요» показывает прошлое время и движение туда, дальше от говорящего.`,

    'main-2':
      `Здесь монета сейчас катится по воде к ногам говорящего.
` +
      `Она движется сюда, поэтому подходит «굴러와요».
` +
      `«와요» показывает движение к месту говорящего.`,

    'main-3':
      `В этом вопросе говорится о том, что может случиться дальше.
` +
      `Если человек отпустит шапку, она улетит к песку.
` +
      `Поэтому нужен будущий ответ «날아갈 거예요»: «날아» — лететь, «갈 거예요» — полетит туда.`,

    'main-4':
      `Бумажный самолётик уже прилетел через окно к столу говорящего.
` +
      `Он двигался по воздуху сюда, поэтому правильный ответ — «날아왔어요».
` +
      `«왔어요» показывает прошлое время и направление к говорящему.`,

    'main-5':
      `Минсу сейчас спешит к остановке.
` +
      `Он бежит в другое место, поэтому подходит «뛰어가요».
` +
      `«뛰어» значит бежать, а «가요» показывает движение туда.`,

    'main-6':
      `Друг ещё не прибежал, но скоро побежит к говорящему.
` +
      `Поэтому нужен будущий ответ «뛰어올 거예요».
` +
      `«올 거예요» показывает, что движение будет сюда.`,

    'main-7':
      `Семья уже дошла пешком до маленькой чайной.
` +
      `Они шли от прежнего места к другому месту, поэтому подходит «걸어갔어요».
` +
      `«갔어요» показывает прошлое время и направление туда.`,

    'main-8':
      `Говорящий сидит у ворот, а друг сейчас идёт к нему.
` +
      `Движение идёт к месту говорящего, поэтому нужен ответ «걸어와요».
` +
      `«걸어» значит пешком, а «와요» показывает движение сюда.`,

    'main-9':
      `Щенок ещё не побежал, но если отпустить поводок, он побежит за кошкой.
` +
      `Кошка впереди, и щенок будет двигаться туда, дальше от хозяина.
` +
      `Поэтому подходит будущая форма «쫓아갈 거예요».`,

    'main-10':
      `Друг уже побежал за Джуни во время игры.
` +
      `Он двигался вслед за ним в сторону рассказчика, поэтому подходит «쫓아왔어요».
` +
      `«왔어요» показывает прошлое время и движение сюда.`,

    'remedial-1':
      `Глиняный шар уже упал и покатился к двери.
` +
      `Он круглый и движется по полу туда, поэтому нужен ответ «굴러갔어요».
` +
      `«갔어요» показывает прошлое время и движение от говорящего.`,

    'remedial-2':
      `Красный мяч сейчас катится с маленького холма к говорящему.
` +
      `Он приближается сюда, поэтому правильный ответ — «굴러와요».
` +
      `«와요» помогает понять направление: к месту говорящего.`,

    'remedial-3':
      `Здесь говорится о возможной ситуации в будущем.
` +
      `Если дети отпустят верёвку, змей улетит к дальнему дереву.
` +
      `Поэтому подходит «날아갈 거예요»: движение будет по воздуху и туда.`,
  };
  ...
  return (
    ...
    (explanations[key] ??
      `В этом вопросе нужно выбрать глагол движения.\n` +
      `Сначала посмотри, как движется предмет или человек.\n` +
      `Потом реши: движение идёт туда — «가다», или сюда — «오다», и проверь время: сейчас, было или будет.`)
  );
}
